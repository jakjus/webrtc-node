...
class RTCDataChannel extends SimpleEventTarget {
  ...
  _sendNativeString(data) {
    ...
    try {
      const sent = this._native.sendString(data);
      if (sent === false && !this._nativeReadyForSend()) {
        throw makeDOMException("RTCDataChannel is not open", "InvalidStateError");
      }
    } catch (error) {
  ...
  _sendNativeBinary(data) {
    ...
    try {
      const sent = this._native.sendBinary(data);
      if (sent === false && !this._nativeReadyForSend()) {
        throw makeDOMException("RTCDataChannel is not open", "InvalidStateError");
      }
    } catch (error) {
...
class RTCPeerConnection extends SimpleEventTarget {
  ...
  close() {
    ...
    if (nativePeer) {
      this._nativeClosePromise = delay(PEER_CONNECTION_NATIVE_CLOSE_DELAY_MS)
        .then(() => {
          try {
            return nativePeer.close();
          } catch {
            return undefined;
          }
        })
