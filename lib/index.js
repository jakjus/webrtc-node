...
function reportListenerError(error) {
  // Per the DOM EventTarget contract, an exception thrown by an event listener
  // must not interrupt dispatch to other listeners or unwind the caller's
  // dispatch loop.
  //
  // This is a hardening boundary: a datachannel message handler that throws on
  // a malformed/hostile packet (a common way to crash game-server hosts) can no
  // longer corrupt the message-flush state machine or stall delivery for other
  // peers — the offending message is dropped and the loop continues.
  //
  // The error is reported WITHOUT re-throwing: re-surfacing it as an
  // uncaughtException would only be non-fatal for hosts that already install an
  // uncaughtException handler (which don't crash anyway), so it would give no
  // crash protection to the hosts that need it most. Instead we emit an opt-in
  // process event for structured handling and fall back to console.error, so a
  // malformed-packet flood is always visibly logged but never fatal.
  if (process.listenerCount("webrtc-node:listenerError") > 0) {
    process.emit("webrtc-node:listenerError", error);
    return;
  }
  // Throttle the console fallback: under a malformed-packet flood, synchronous
  // per-message stderr writes would themselves become a backpressure DoS. Log
  // at most once per second with a dropped-count summary.
  kListenerErrorState.suppressed += 1;
  const now = Date.now();
  if (now - kListenerErrorState.lastLogged < 1000) return;
  const dropped = kListenerErrorState.suppressed - 1;
  kListenerErrorState.suppressed = 0;
  kListenerErrorState.lastLogged = now;
  console.error(
    `[webrtc-node] event listener threw (message dropped${
      dropped > 0 ? `, +${dropped} more suppressed in last 1s` : ""
    }):`,
    error,
  );
}

const kListenerErrorState = { lastLogged: 0, suppressed: 0 };
