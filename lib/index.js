...
class RTCDataChannel extends SimpleEventTarget {
  ...
  constructor(peerConnection, nativeChannel, initialReadyState = undefined, assignedId = null) {
    ...
    this._nativeSendString = (nativeChannel.sendStringFast || nativeChannel.sendString).bind(
      nativeChannel,
    );
    this._nativeSendBinary = (nativeChannel.sendBinaryFast || nativeChannel.sendBinary).bind(
      nativeChannel,
    );
    this._readyState =
      initialReadyState ||
  ...
  _sendNativeString(data) {
    ...
    try {
      let sent = this._nativeSendString(data);
      if (sent !== false) {
        sent = true;
      } else {
  ...
  _sendNativeBinary(data) {
    ...
    try {
      const view = data instanceof Uint8Array ? data : toUint8Array(data);
      let sent = this._nativeSendBinary(view);
      if (sent !== false) {
        sent = true;
      } else {
        sent = Boolean(this._native?.isOpen);
