...
class RTCDataChannel extends SimpleEventTarget {
  ...
  _sendNativeString(data) {
    ...
    try {
      return this._tryNativeSend(() => this._native.sendString(data));
    } catch (error) {
  ...
  _sendNativeBinary(data) {
    ...
    try {
      return this._tryNativeSend(() => this._native.sendBinary(data));
    } catch (error) {
      throw mapNativeError(error, "OperationError");
    }
  }
  ...
  _tryNativeSend(send) {
    const bufferedBefore = Number(this._native?.bufferedAmount || 0);
    const sent = send();
    if (sent !== false) return true;
    return Number(this._native?.bufferedAmount || 0) > bufferedBefore;
  }
