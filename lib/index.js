...
const MESSAGE_EVENT_FLUSH_BATCH_LIMIT = 4096;
...
class RTCDataChannel extends SimpleEventTarget {
  ...
  _flushQueuedMessageEvents() {
    let delivered = 0;
    while (this._queuedMessageEvents.length) {
      ...
      delivered += 1;
      if (delivered >= MESSAGE_EVENT_FLUSH_BATCH_LIMIT && this._queuedMessageEvents.length > 0) {
        this._scheduleMessageEventFlush();
        return;
      }
    }
